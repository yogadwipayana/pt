...
const hasDwipaConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["dwipa"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["dwipa"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasDwipa: hasDwipaConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("dwipa/") ? config.model.replace(/^dwipa\//, "") : null,
...
// POST - Apply Dwipa as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_dwipa";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing dwipa provider entry and its models
    const existingProvider = config.provider["dwipa"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["dwipa"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `dwipa/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `dwipa/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("dwipa/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Dwipa provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["dwipa"]?.models) {
      delete config.provider["dwipa"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["dwipa"].models).length === 0) {
        delete config.provider["dwipa"];
        if (config.model?.startsWith("dwipa/")) delete config.model;
      } else if (config.model === `dwipa/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["dwipa"].models);
        config.model = `dwipa/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire dwipa provider
      if (config.provider) delete config.provider["dwipa"];
      if (config.model?.startsWith("dwipa/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("dwipa/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Dwipa settings removed from OpenCode",
    });
  } catch (error) {
