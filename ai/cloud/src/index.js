...
import { handleAdminData } from "./handlers/adminData.js";
...
const worker = {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      // Routes
      if (path.startsWith("/api/web/v1/admin/auth/")) {
        const response = await handleAdminAuth(request, env, path);
        log.response(response.status, Date.now() - startTime);
        return response;
      }

      if (path.startsWith("/api/web/v1/admin/")) {
        const response = await handleAdminData(request, env, path);
        log.response(response.status, Date.now() - startTime);
        return response;
      }
