...
// Check if settings has Dwipa customModels
const hasDwipaConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Dwipa"));
};
...
// POST - Update Dwipa customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Dwipa configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Dwipa"));
...
// DELETE - Remove Dwipa customModels only (keep other settings)
