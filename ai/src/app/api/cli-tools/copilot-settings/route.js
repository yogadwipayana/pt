...
const hasDwipaConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Dwipa");
};

const getDwipaEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Dwipa") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getDwipaEntry(config);
...
// POST - Apply Dwipa config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Dwipa entry or append
    const idx = config.findIndex((e) => e.name === "Dwipa");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Dwipa entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Dwipa");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Dwipa removed from Copilot config",
    });
  } catch (error) {
