...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("dwipa/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("dwipa/", ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    const url = status.config?.provider?.["dwipa"]?.options?.baseURL || "";
    const isLocal = url.includes("localhost") || url.includes("127.0.0.1");
    return status.hasDwipa && (isLocal || url.includes(baseUrl)) ? "configured" : status.hasDwipa ? "other" : "not_configured";
  };
  ...
  const handleApply = async () => {
    ...
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_dwipa" : selectedApiKey);
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current base URL */}
                {status?.config?.provider?.["dwipa"]?.options?.baseURL && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasDwipa} loading={restoring}>
