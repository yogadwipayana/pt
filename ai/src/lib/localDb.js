...
/* ------------------------------------------------------------------
   API Key persistence: prefer PostgreSQL (DATABASE_URL), fallback to db.json
   ------------------------------------------------------------------ */
...
async function pgApiKeys() {
  const { getApiKeys: pgGetApiKeys } = await import("@/lib/adminPostgres.js");
  return pgGetApiKeys();
}

export async function getApiKeys() {
  if (hasPostgres()) return pgApiKeys();
  const db = await getDb();
...
export async function createUser(data) {
  ...
  const user = {
    id: data.id || uuidv4(),
    email,
    fullName: String(data?.fullName || "").trim() || email.split("@")[0] || "dwipa",
    passwordHash: data?.passwordHash || null,
    planSlug: data?.planSlug || "free",
    creditBalanceUsd: Number.isFinite(Number(data?.creditBalanceUsd)) ? Number(data.creditBalanceUsd) : 0,
    createdAt: now,
    updatedAt: now,
  };
...
export async function updateUserByEmail(email, updates) {
  ...
  const db = await getDb();
  const index = (db.data.users || []).findIndex((user) => user.email === normalizedEmail);
  if (index === -1) return null;
...
export async function createApiKey(name, machineId, userId = null) {
  ...
  if (hasPostgres()) {
    const { createApiKey: pgCreateApiKey } = await import("@/lib/adminPostgres.js");
    return pgCreateApiKey(name, machineId, userId);
  }
  const db = await getDb();
  const now = new Date().toISOString();
...
export async function deleteApiKey(id) {
  if (hasPostgres()) {
    const { deleteApiKey: pgDeleteApiKey } = await import("@/lib/adminPostgres.js");
    return pgDeleteApiKey(id);
  }
  const db = await getDb();
...
export async function getApiKeyById(id) {
  if (hasPostgres()) {
    const { getApiKeyById: pgGetApiKeyById } = await import("@/lib/adminPostgres.js");
    return pgGetApiKeyById(id);
  }
  const db = await getDb();
...
export async function getApiKeyByValue(key) {
  if (hasPostgres()) {
    const { getApiKeyByValue: pgGetApiKeyByValue } = await import("@/lib/adminPostgres.js");
    return pgGetApiKeyByValue(key);
  }
  const db = await getDb();
...
export async function updateApiKey(id, data) {
  if (hasPostgres()) {
    const { updateApiKey: pgUpdateApiKey } = await import("@/lib/adminPostgres.js");
    return pgUpdateApiKey(id, data);
  }
  const db = await getDb();
  const index = db.data.apiKeys.findIndex(k => k.id === id);
  if (index === -1) return null;
...
export async function validateApiKey(key) {
  if (hasPostgres()) {
    const { validateApiKey: pgValidateApiKey } = await import("@/lib/adminPostgres.js");
    return pgValidateApiKey(key);
  }
  const db = await getDb();
  const found = db.data.apiKeys.find(k => k.key === key);
  return Boolean(found && found.isActive !== false && !found.revokedAt);
}
...
export async function exportDb() {
  ...
  // When PostgreSQL is configured, API keys live there — exclude from db.json backup
  // so import/restore does not overwrite Postgres state.
  if (hasPostgres()) {
...
export async function importDb(payload) {
  ...
  // When PostgreSQL is configured, ignore apiKeys from backup; they live in Postgres.
  if (hasPostgres()) {
    delete nextData.apiKeys;
  }
...
export async function getPricingForModel(provider, model) {
  ...
  // Admin ModelCatalog is source of truth for input/output prices
  try {
    const { dbGetAdminModelPricing } = await import("@/lib/adminPostgres.js");
    const adminPricing = await dbGetAdminModelPricing(`${provider}/${model}`);
    if (adminPricing && (adminPricing.input > 0 || adminPricing.output > 0)) {
      const { getPricingForModel: resolve } = await import("@/shared/constants/pricing.js");
      const constantPricing = resolve(provider, model) || {};
      return {
        input: adminPricing.input,
        output: adminPricing.output,
        cached: constantPricing.cached || adminPricing.input,
        reasoning: constantPricing.reasoning || adminPricing.output,
        cache_creation: constantPricing.cache_creation || adminPricing.input,
      };
    }
  } catch {
    // Admin pricing unavailable — fall through to constants
  }
