...
export async function handleVerify(request, env, machineIdOverride = null) {
  ...
  const machineKey = data.apiKeys?.find(k => k.key === apiKey && k.isActive !== false && !k.revokedAt) || null;
  const dbKey = env.DB && machineKey ? await env.DB.prepare("SELECT k.id FROM api_keys k WHERE k.machineId = ? AND (k.maskedKey = ? OR k.label = ?) AND k.revokedAt IS NULL")
    .bind(machineId, machineKey.maskedKey || "", machineKey.label || machineKey.name || "")
    .first() : null;
  const isValid = Boolean(machineKey && (!env.DB || dbKey));
